...
public class Installer {
	...
	protected void internalInstallAndRun(final ResourceLoader resources, final ModpackModel pack, final String build, final boolean doFullInstall, final LauncherFrame fraeme, final DownloadListener listener, final boolean doLaunch) {
		runningThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if (doLaunch) {
						...
						if (data != null && !data.isRunDataValid(memory, versionNumber)) {
							System.err.println("CANNOT LAUNCH MODPACK. REQUIRES AT LEAST: " + memory + "MB OF MEMORY!");
				...
				//					JOptionPane.showMessageDialog(frame, resources.getString("launcher.installerror.download", pack.getDisplayName(), e.getMessage()),
				//							resources.getString("launcher.installerror.title"), JOptionPane.WARNING_MESSAGE);
				//				} catch (ZipException e) {
				//					JOptionPane.showMessageDialog(frame, resources.getString("launcher.installerror.unzip", pack.getDisplayName(), e.getMessage()), resources.getString("launcher.installerror.title"),
				//							JOptionPane.WARNING_MESSAGE);
				//				} catch (CacheDeleteException e) {
				//					JOptionPane.showMessageDialog(frame, resources.getString("launcher.installerror.cache", pack.getDisplayName(), e.getMessage()), resources.getString("launcher.installerror.title"),
				//							JOptionPane.WARNING_MESSAGE);
	...
	public void buildTasksQueue(InstallTasksQueue queue, ResourceLoader resources, ModpackModel modpack, String build, boolean doFullInstall, MojangVersionBuilder versionBuilder) throws CacheDeleteException, BuildInaccessibleException {
		...
		if (minecraft.startsWith("1.5")) {
			verifyingFiles.addTask(new EnsureFileTask(new File(directories.getCacheDirectory(), "fml_libs15.zip"), new ValidZipFileVerifier(), new File(modpack.getInstalledDirectory(), "lib"),
					"http://mirror.technicpack.net/Technic/lib/fml/fml_libs15.zip", installingLibs, installingLibs));
		} else if (minecraft.startsWith("1.4")) {
			verifyingFiles.addTask(new EnsureFileTask(new File(directories.getCacheDirectory(), "fml_libs.zip"), new ValidZipFileVerifier(), new File(modpack.getInstalledDirectory(), "lib"),
					"http://mirror.technicpack.net/Technic/lib/fml/fml_libs.zip", installingLibs, installingLibs));
		}
